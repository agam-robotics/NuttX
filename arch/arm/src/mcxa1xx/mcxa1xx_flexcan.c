/****************************************************************************
 * arch/arm/src/mcxa1xx/mcxa1xx_flexcan.c
 *
...
#include "chip.h"
#include "hardware/mcxa1xx_flexcan.h"
#include "mcxa1xx_clockconfig.h"
#include "mcxa1xx_periphclock.h"
#include "mcxa1xx_port.h"
#include "mcxa1xx_flexcan.h"

#include <arch/board/board.h>

#include <sys/time.h>

#ifdef CONFIG_MCXA1XX_FLEXCAN
...
/* CONFIG_MCXA1XX_FLEXCAN_NETHIFS determines the number of physical
...
{
  port_cfg_t tx_pin;         /* PORT configuration for TX */
  port_cfg_t rx_pin;         /* PORT configuration for RX */
  uint32_t no_buffers;       /* Number of message buffers */
  port_cfg_t enable_pin;     /* Optional enable pin */
  uint32_t enable_high;      /* Optional enable high/low */
  port_cfg_t stb_pin;        /* Optional stb pin */
  uint32_t stb_high;         /* Optional stb high/low */
  port_cfg_t led_pin;        /* Optional led pin */
...
#ifdef CONFIG_MCXA1XX_FLEXCAN0
static const struct flexcan_config_s mcxa1xx_flexcan0_config =
{
  .tx_pin      = PIN_CAN0_TX,
  .rx_pin      = PIN_CAN0_RX,
  .no_buffers  = 32,
  ...
  .led_high = 0,
#endif
  /* This part routes every FlexCAN event to one vector, unlike the S32K3XX
   * which splits bus/error/lprx/mb. Only mb_irq is used: the shared handler
   * acts solely on the MB flags, so the other vectors are no-ops upstream
   * too and leaving them zero loses nothing.
   */

  .bus_irq     = 0,
  .error_irq   = 0,
  .lprx_irq    = 0,
  .mb_irq      = MCXA156_IRQ_CAN0,
};
#endif
...
/* The mcxa1xx_driver_s encapsulates all state information for a single
 * hardware interface
 */

struct mcxa1xx_driver_s
{
...
#ifdef CONFIG_MCXA1XX_FLEXCAN0
static struct mcxa1xx_driver_s g_flexcan0;
#endif
...
uint32_t mcxa1xx_bitratetotimeseg(struct flexcan_timeseg *timeseg,
...
static bool mcxa1xx_txringfull(struct mcxa1xx_driver_s *priv);
static int  mcxa1xx_transmit(struct mcxa1xx_driver_s *priv);
static int  mcxa1xx_txpoll(struct net_driver_s *dev);

/* Helper functions */

static void mcxa1xx_setenable(uint32_t base, uint32_t enable);
static void mcxa1xx_setfreeze(uint32_t base, uint32_t freeze);
static uint32_t mcxa1xx_waitmcr_change(uint32_t base,
                                       uint32_t mask,
                                       uint32_t target_state);

/* Interrupt handling */

static void mcxa1xx_receive(struct mcxa1xx_driver_s *priv, uint32_t flags);
static void mcxa1xx_txdone_work(void *arg);
static void mcxa1xx_txdone(struct mcxa1xx_driver_s *priv);

static int  mcxa1xx_flexcan_interrupt(int irq, void *context, void *arg);

/* Watchdog timer expirations */
#ifdef TX_TIMEOUT_WQ
static void mcxa1xx_txtimeout_work(void *arg);
static void mcxa1xx_txtimeout_expiry(wdparm_t arg);
#endif

/* NuttX callback functions */

static int  mcxa1xx_ifup(struct net_driver_s *dev);
static int  mcxa1xx_ifdown(struct net_driver_s *dev);

static void mcxa1xx_txavail_work(void *arg);
static int  mcxa1xx_txavail(struct net_driver_s *dev);

#ifdef CONFIG_NETDEV_IOCTL
static int  mcxa1xx_ioctl(struct net_driver_s *dev, int cmd,
                          unsigned long arg);
#endif

/* Initialization */

static int  mcxa1xx_initialize(struct mcxa1xx_driver_s *priv);
static void mcxa1xx_reset(struct mcxa1xx_driver_s *priv);
...
static int mcxa1xx_transmit(struct mcxa1xx_driver_s *priv)
{
  ...
  if ((getreg32(priv->base + MCXA1XX_CAN_ESR2_OFFSET) &
      (CAN_ESR2_IMB | CAN_ESR2_VPS)) ==
      (CAN_ESR2_IMB | CAN_ESR2_VPS))
    {
      mbi  = ((getreg32(priv->base + MCXA1XX_CAN_ESR2_OFFSET) &
  ...
  while (mbi < TXMBCOUNT)
    {
      if (priv->tx[mbi].cs.code != CAN_TXMB_DATAORREMOTE)
        {
          putreg32(mb_bit, priv->base + MCXA1XX_CAN_IFLAG1_OFFSET);
          break;
        }
  ...
  regval = getreg32(priv->base + MCXA1XX_CAN_IMASK1_OFFSET);
  regval |= mb_bit;
  putreg32(regval, priv->base + MCXA1XX_CAN_IMASK1_OFFSET);
  ...
  if (timeout >= 0)
    {
      wd_start(&priv->txtimeout[mbi], timeout + 1,
               mcxa1xx_txtimeout_expiry, (wdparm_t)priv);
    }
#endif
...
static int mcxa1xx_txpoll(struct net_driver_s *dev)
{
  struct mcxa1xx_driver_s *priv = (struct mcxa1xx_driver_s *)dev->d_private;
  ...
  while ((mb_index = arm_lsb(flags)) != 32)
    {
      ...
      if (rf->cs.edl)
        {
          ...
          putreg32(1 << mb_index,
                   priv->base + MCXA1XX_CAN_IFLAG1_OFFSET);
      ...
      if (flags == 0)
        {
          flags  = getreg32(priv->base + MCXA1XX_CAN_IFLAG1_OFFSET);
          flags &= IFLAG1_RX;
        }
    }
}
...
static void mcxa1xx_txdone_work(void *arg)
{
  struct mcxa1xx_driver_s *priv = (struct mcxa1xx_driver_s *)arg;
  ...
  net_lock();
  devif_poll(&priv->dev, mcxa1xx_txpoll);
  net_unlock();
}

/****************************************************************************
 * Function: mcxa1xx_flexcan_interrupt
 *
 * Description:
      ...
      if (flags)
        {
          ...
          flags  = getreg32(priv->base + MCXA1XX_CAN_IMASK1_OFFSET);
          flags &= ~(IFLAG1_TX);
          putreg32(flags, priv->base + MCXA1XX_CAN_IMASK1_OFFSET);
          work_queue(CANWORK, &priv->irqwork, mcxa1xx_txdone_work, priv, 0);
        }
    }
...
/****************************************************************************
 * Function: mcxa1xx_txtimeout_work
 *
 * Description:
...
/****************************************************************************
 * Function: mcxa1xx_txtimeout_expiry
 *
 * Description:
...
static void mcxa1xx_txtimeout_expiry(wdparm_t arg)
{
  ...
  work_queue(CANWORK, &priv->irqwork, mcxa1xx_txtimeout_work, priv, 0);
}
...
static void mcxa1xx_setenable(uint32_t base, uint32_t enable)
{
  ...
  if (enable)
    {
      regval  = getreg32(base + MCXA1XX_CAN_MCR_OFFSET);
      regval &= ~(CAN_MCR_MDIS);
      putreg32(regval, base + MCXA1XX_CAN_MCR_OFFSET);
    }
  else
    {
      regval  = getreg32(base + MCXA1XX_CAN_MCR_OFFSET);
  ...
  mcxa1xx_waitmcr_change(base, CAN_MCR_LPMACK, 1);
}
...
static uint32_t mcxa1xx_waitmcr_change(uint32_t base, uint32_t mask,
                                       uint32_t target_state)
{
  ...
  for (wait_ack = 0; wait_ack < timeout; wait_ack++)
    {
      const bool state = (getreg32(base + MCXA1XX_CAN_MCR_OFFSET) & mask)
          != 0;
...
static uint32_t mcxa1xx_waitfreezeack_change(uint32_t base,
                                             uint32_t target_state)
{
  return mcxa1xx_waitmcr_change(base, CAN_MCR_FRZACK, target_state);
}

/****************************************************************************
 * Function: mcxa1xx_ifup
 *
 * Description:
...
static int mcxa1xx_ifup(struct net_driver_s *dev)
{
  ...
  mcxa1xx_setfreeze(priv->base, 0);
  if (!mcxa1xx_waitfreezeack_change(priv->base, 0))
    {
  ...
  if (priv->config->led_pin > 0)
    {
      mcxa1xx_port_configure(priv->config->led_pin);
      mcxa1xx_gpiowrite(priv->config->led_pin, priv->config->led_high);
    }
      ...
      case SIOCSCANBITRATE: /* Set bitrate of a CAN controller */
        {
          ...
          if (mcxa1xx_bitratetotimeseg(&arbi_timing, 10, 0,
                                       priv->clk_freq))
            {
              ret = OK;
            }
          ...
          if (ret == OK && mcxa1xx_bitratetotimeseg(&data_timing, 10, 1,
                                                    priv->clk_freq))
            {
              ret = OK;
            }
...
static int mcxa1xx_init_eccram(struct mcxa1xx_driver_s *priv)
{
  uint32_t i;
  uint32_t regval;
  irqstate_t flags;
  ...
  regval = getreg32(priv->base + MCXA1XX_CAN_CTRL2_OFFSET);

  /* Set WRMFRZ bit in CTRL2 Register to grant write access to memory */

  regval |= CAN_CTRL2_WRMFRZ;

  putreg32(regval, priv->base + MCXA1XX_CAN_CTRL2_OFFSET);

  for (i = MCXA1XX_CAN_MB_OFFSET; i < 0xadf + 1; i += 4)
    {
...
static int mcxa1xx_initialize(struct mcxa1xx_driver_s *priv)
{
  ...
  mcxa1xx_setenable(priv->base, 1);

  mcxa1xx_init_eccram(priv);

  mcxa1xx_reset(priv);

  /* Enter freeze mode */

  mcxa1xx_setfreeze(priv->base, 1);
  if (!mcxa1xx_waitfreezeack_change(priv->base, 1))
    {
  ...
  regval  = getreg32(priv->base + MCXA1XX_CAN_CTRL1_OFFSET);
  regval &= ~(CAN_CTRL1_LOM | CAN_CTRL1_LBUF | CAN_CTRL1_TSYN |
              ...
              CAN_CTRL1_BOFFMSK);
  putreg32(regval, priv->base + MCXA1XX_CAN_CTRL1_OFFSET);
  ...
  putreg32(regval, priv->base + MCXA1XX_CAN_CBT_OFFSET);

  /* Enable CAN FD feature */

  regval  = getreg32(priv->base + MCXA1XX_CAN_MCR_OFFSET);
  regval |= CAN_MCR_FDEN;
  putreg32(regval, priv->base + MCXA1XX_CAN_MCR_OFFSET);
  ...
  putreg32(regval, priv->base + MCXA1XX_CAN_FDCBT_OFFSET);
  ...
  putreg32(regval, priv->base + MCXA1XX_CAN_FDCTRL_OFFSET);
  ...
  putreg32(0x0, priv->base + MCXA1XX_CAN_RXFGMASK_OFFSET);

  for (i = 0; i < priv->config->no_buffers; i++)
    {
      putreg32(0, priv->base + MCXA1XX_CAN_RXIMR_OFFSET(i));
    }
  ...
  putreg32(IFLAG1_RX, priv->base + MCXA1XX_CAN_IFLAG1_OFFSET);
  putreg32(IFLAG1_RX, priv->base + MCXA1XX_CAN_IMASK1_OFFSET);
...
static void mcxa1xx_reset(struct mcxa1xx_driver_s *priv)
{
  ...
  if (!mcxa1xx_waitmcr_change(priv->base, CAN_MCR_SOFTRST, 0))
    {
      nerr("Reset failed");
      return;
    }
  ...
  putreg32(0x3fffffff, priv->base + MCXA1XX_CAN_RX14MASK_OFFSET);
  putreg32(0x3fffffff, priv->base + MCXA1XX_CAN_RX15MASK_OFFSET);
  putreg32(0x3fffffff, priv->base + MCXA1XX_CAN_RXMGMASK_OFFSET);
  putreg32(0x0, priv->base + MCXA1XX_CAN_RXFGMASK_OFFSET);
}
...
/****************************************************************************
 * Function: mcxa1xx_caninitialize
 *
 * Description:
...
int mcxa1xx_caninitialize(int intf)
{
  struct mcxa1xx_driver_s *priv;
  int ret;
  ...
  switch (intf)
    ...
    case 0:
      priv               = &g_flexcan0;
      memset(priv, 0, sizeof(struct mcxa1xx_driver_s));
      priv->base         = MCXA1XX_FLEXCAN0_BASE;
      priv->config       = &mcxa1xx_flexcan0_config;
      priv->clk_freq     = BOARD_FLEXCAN_FREQUENCY;

      /* Select the FlexCAN functional clock and open its gate. The source
       * must be selected before the gate is opened.
       */

      mcxa1xx_set_periphclock(MCXA1XX_MRCC_FLEXCAN0_CLKSEL,
                              MCXA1XX_MRCC_FLEXCAN0_CLKDIV,
                              BOARD_FLEXCAN_CLKSEL, BOARD_FLEXCAN_CLKDIV);
      mcxa1xx_set_clock_gate(CLOCK_GATE_FLEXCAN0, true);
...
#  endif
  ...
  if (!mcxa1xx_bitratetotimeseg(&priv->arbi_timing, 1, 0, priv->clk_freq))
    {
...
#ifdef CONFIG_NET_CAN_CANFD
  if (!mcxa1xx_bitratetotimeseg(&priv->data_timing, 1, 1, priv->clk_freq))
    {
  ...
  mcxa1xx_port_configure(priv->config->tx_pin);
  mcxa1xx_port_configure(priv->config->rx_pin);
  if (priv->config->enable_pin > 0)
    {
      mcxa1xx_port_configure(priv->config->enable_pin);
      mcxa1xx_gpiowrite(priv->config->enable_pin, priv->config->enable_high);
    }

  if (priv->config->stb_pin > 0)
    {
      mcxa1xx_port_configure(priv->config->stb_pin);
      mcxa1xx_gpiowrite(priv->config->stb_pin, priv->config->stb_high);
    }
  ...
  if (priv->config->bus_irq > 0)
      {
      if (irq_attach(priv->config->bus_irq, mcxa1xx_flexcan_interrupt, priv))
        {
  ...
  if (priv->config->error_irq > 0)
      ...
      if (irq_attach(priv->config->error_irq,
                     mcxa1xx_flexcan_interrupt, priv))
        {
          /* We could not attach the ISR to the interrupt */
  ...
  if (irq_attach(priv->config->mb_irq, mcxa1xx_flexcan_interrupt, priv))
    {
      /* We could not attach the ISR to the interrupt */
  ...
  priv->dev.d_ifup    = mcxa1xx_ifup;      /* I/F up (new IP address) callback */
  priv->dev.d_ifdown  = mcxa1xx_ifdown;    /* I/F down callback */
  priv->dev.d_txavail = mcxa1xx_txavail;   /* New TX data callback */
#ifdef CONFIG_NETDEV_IOCTL
  priv->dev.d_ioctl   = mcxa1xx_ioctl;     /* Support CAN ioctl() calls */
#endif
  priv->dev.d_private = priv;              /* Used to recover private state from dev */
  priv->rx            = (struct mb_s *)(priv->base + MCXA1XX_CAN_MB_OFFSET);
  priv->tx            = (struct mb_s *)(priv->base + MCXA1XX_CAN_MB_OFFSET +
                          (sizeof(struct mb_s) * RXMBCOUNT));

  /* Put the interface in the down state.  This usually amounts to resetting
   * the device and/or calling mcxa1xx_ifdown().
   */

  ninfo("callbacks done\n");

  mcxa1xx_initialize(priv);

  mcxa1xx_ifdown(&priv->dev);
