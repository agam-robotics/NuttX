/****************************************************************************
 * arch/arm/src/mcxa1xx/mcxa1xx_start.c
 *
...
void __start(void)
{
   ...
   * at _framfuncs.  This should be done before mcxa1xx_clockconfig() is
  ...
  mcxa1xx_boardinitialize();

  /* No cache is enabled here. This part has a 4 KB flash accelerator
   * cache, but no ARM architectural I- or D-cache (CONFIG_ARCH_ICACHE is
   * not selected for this chip), so up_enable_icache()/up_enable_dcache()
   * would be no-ops.
   */
