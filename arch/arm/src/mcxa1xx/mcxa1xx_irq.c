/****************************************************************************
 * arch/arm/src/mcxa1xx/mcxa1xx_irq.c
 *
...
static void mcxa1xx_dumpnvic(const char *msg, int irq)
{
  irqstate_t flags;
...
#else
#  define mcxa1xx_dumpnvic(msg, irq)
#endif

/****************************************************************************
 * Name: mcxa1xx_nmi, mcxa1xx_pendsv, mcxa1xx_pendsv, mcxa1xx_reserved
 *
 * Description:
...
static int mcxa1xx_nmi(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int mcxa1xx_pendsv(int irq, void *context, void *arg)
{
  up_irq_save();
...
static int mcxa1xx_reserved(int irq, void *context, void *arg)
{
  up_irq_save();
...
/****************************************************************************
 * Name: mcxa1xx_prioritize_syscall
 *
 * Description:
...
static inline void mcxa1xx_prioritize_syscall(int priority)
{
  uint32_t regval;
...
static int mcxa1xx_irqinfo(int irq, uintptr_t *regaddr, uint32_t *bit,
                       uintptr_t offset)
{
  unsigned int extint = irq - MCXA1XX_IRQ_EXTINT;

  DEBUGASSERT(irq >= MCXA1XX_IRQ_NMI && irq < NR_IRQS);

  /* Check for external interrupt */

  if (irq >= MCXA1XX_IRQ_EXTINT)
    {
      if (extint < 32)
        {
  ...
  /* Handle processor exceptions.  Only a few can be disabled */
  ...
  else
    {
      *regaddr = NVIC_SYSHCON;
      if (irq == MCXA1XX_IRQ_MEMFAULT)
        {
          *bit = NVIC_SYSHCON_MEMFAULTENA;
        }
      else if (irq == MCXA1XX_IRQ_BUSFAULT)
        {
          *bit = NVIC_SYSHCON_BUSFAULTENA;
        }
      else if (irq == MCXA1XX_IRQ_USAGEFAULT)
        {
          *bit = NVIC_SYSHCON_USGFAULTENA;
        }
      else if (irq == MCXA1XX_IRQ_SYSTICK)
        {
...
void up_irqinitialize(void)
{
  ...
  irq_attach(MCXA1XX_IRQ_SVCALL, arm_svcall, NULL);
  irq_attach(MCXA1XX_IRQ_HARDFAULT, arm_hardfault, NULL);

  /* Set the priority of the SVCall interrupt */

#ifdef CONFIG_ARCH_IRQPRIO
  /* up_prioritize_irq(MCXA1XX_IRQ_PENDSV, NVIC_SYSH_PRIORITY_MIN); */
#endif
  mcxa1xx_prioritize_syscall(NVIC_SYSH_SVCALL_PRIORITY);
  ...
  irq_attach(MCXA1XX_IRQ_MEMFAULT, arm_memfault, NULL);
  up_enable_irq(MCXA1XX_IRQ_MEMFAULT);
#endif

  /* Attach all other processor exceptions (except reset and sys tick) */

#ifdef CONFIG_DEBUG_FEATURES
  irq_attach(MCXA1XX_IRQ_NMI, mcxa1xx_nmi, NULL);
#ifndef CONFIG_ARM_MPU
  irq_attach(MCXA1XX_IRQ_MEMFAULT, arm_memfault, NULL);
#endif
  irq_attach(MCXA1XX_IRQ_BUSFAULT, arm_busfault, NULL);
  irq_attach(MCXA1XX_IRQ_USAGEFAULT, arm_usagefault, NULL);
  irq_attach(MCXA1XX_IRQ_PENDSV, mcxa1xx_pendsv, NULL);
  arm_enable_dbgmonitor();
  irq_attach(MCXA1XX_IRQ_DBGMONITOR, arm_dbgmonitor, NULL);
  irq_attach(MCXA1XX_IRQ_RESERVED, mcxa1xx_reserved, NULL);
#endif

  mcxa1xx_dumpnvic("initial", NR_IRQS);
...
void up_disable_irq(int irq)
{
  ...
  if (mcxa1xx_irqinfo(irq, &regaddr, &bit, NVIC_CLRENA_OFFSET) == 0)
    {
  ...
  else
    {
      ...
      mcxa1xx_gpioirq_disable(irq);
    }
#endif

#if 0 /* Might be useful in early bring-up */
  mcxa1xx_dumpnvic("disable", irq);
#endif
}
...
void up_enable_irq(int irq)
{
  ...
  if (mcxa1xx_irqinfo(irq, &regaddr, &bit, NVIC_ENA_OFFSET) == 0)
    {
  ...
  else
    {
      ...
      mcxa1xx_gpioirq_enable(irq);
    }
#endif

#if 0 /* Might be useful in early bring-up */
  mcxa1xx_dumpnvic("enable", irq);
#endif
}
...
int up_prioritize_irq(int irq, int priority)
{
  ...
  DEBUGASSERT(irq >= MCXA1XX_IRQ_MEMFAULT && irq < NR_IRQS &&
  ...
  mcxa1xx_dumpnvic("prioritize", irq);
  return OK;
}
#endif
