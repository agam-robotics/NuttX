/****************************************************************************
 * arch/arm/src/mcxa1xx/mcxa1xx_lpuart.c
 *
...
#include "hardware/mcxa1xx_lpuart.h"
#include "hardware/mcxa1xx_port.h"
#include "mcxa1xx_lowputc.h"
#include "mcxa1xx_serial.h"

#if defined(SERIAL_HAVE_TXDMA) || defined(SERIAL_HAVE_RXDMA)
#  include "chip.h"
#  include "mcxa1xx_edma.h"
#  include "hardware/mcxa1xx_dmamux.h"
#endif
...
#define RXDMA_BUFFER_SIZE   ((CONFIG_MCXA1XX_SERIAL_RXDMA_BUFFER_SIZE \
                                + RXDMA_BUFFER_MASK) & ~RXDMA_BUFFER_MASK)
#define TXDMA_BUFFER_MASK   (DMA_ALIGNMENT - 1)
#define TXDMA_BUFFER_SIZE   ((CONFIG_MCXA1XX_SERIAL_RXDMA_BUFFER_SIZE \
                              + RXDMA_BUFFER_MASK) & ~RXDMA_BUFFER_MASK)
...
#if defined(CONFIG_PM) && !defined(CONFIG_MCXA1XX_PM_SERIAL_ACTIVITY)
#  define CONFIG_MCXA1XX_PM_SERIAL_ACTIVITY 10
#endif
...
static inline uint32_t mcxa1xx_serialin(struct mcxa1xx_uart_s *priv,
                                      uint32_t offset);
static inline void mcxa1xx_serialout(struct mcxa1xx_uart_s *priv,
                                   uint32_t offset, uint32_t value);
static inline void mcxa1xx_disableuartint(struct mcxa1xx_uart_s *priv,
                                        uint32_t *ie);
static inline void mcxa1xx_restoreuartint(struct mcxa1xx_uart_s *priv,
                                        uint32_t ie);

static int  mcxa1xx_setup(struct uart_dev_s *dev);
static void mcxa1xx_shutdown(struct uart_dev_s *dev);
static int  mcxa1xx_attach(struct uart_dev_s *dev);
static void mcxa1xx_detach(struct uart_dev_s *dev);
static int  mcxa1xx_interrupt(int irq, void *context, void *arg);
static int  mcxa1xx_ioctl(struct file *filep, int cmd, unsigned long arg);
#if !defined(SERIAL_HAVE_ONLY_RXDMA)
static int  mcxa1xx_receive(struct uart_dev_s *dev, unsigned int *status);
static void mcxa1xx_rxint(struct uart_dev_s *dev, bool enable);
static bool mcxa1xx_rxavailable(struct uart_dev_s *dev);
#endif
#if !defined(SERIAL_HAVE_ONLY_TXDMA)
static void mcxa1xx_txint(struct uart_dev_s *dev, bool enable);
#endif

static void mcxa1xx_send(struct uart_dev_s *dev, int ch);

static bool mcxa1xx_txready(struct uart_dev_s *dev);

#ifdef SERIAL_HAVE_TXDMA
static void mcxa1xx_dma_send(struct uart_dev_s *dev);
static void mcxa1xx_dma_txint(struct uart_dev_s *dev, bool enable);
static void mcxa1xx_dma_txavailable(struct uart_dev_s *dev);
static void mcxa1xx_dma_txcallback(DMACH_HANDLE handle, void *arg, bool done,
                                   int result);
#endif

#if defined(SERIAL_HAVE_RXDMA) || defined(SERIAL_HAVE_TXDMA)
static int  mcxa1xx_dma_setup(struct uart_dev_s *dev);
static void mcxa1xx_dma_shutdown(struct uart_dev_s *dev);
#endif

#ifdef SERIAL_HAVE_RXDMA
static int  mcxa1xx_dma_receive(struct uart_dev_s *dev,
                                unsigned int *status);
#ifdef CONFIG_PM
static void mcxa1xx_dma_reenable(struct mcxa1xx_uart_s *priv);
#endif
static void mcxa1xx_dma_rxint(struct uart_dev_s *dev, bool enable);
static bool mcxa1xx_dma_rxavailable(struct uart_dev_s *dev);

static void mcxa1xx_dma_rxcallback(DMACH_HANDLE handle, void *arg, bool done,
                                   int result);
#endif

static bool mcxa1xx_txempty(struct uart_dev_s *dev);
...
{
  .setup          = mcxa1xx_setup,
  .shutdown       = mcxa1xx_shutdown,
  .attach         = mcxa1xx_attach,
  .detach         = mcxa1xx_detach,
  .ioctl          = mcxa1xx_ioctl,
  .receive        = mcxa1xx_receive,
  .rxint          = mcxa1xx_rxint,
  .rxavailable    = mcxa1xx_rxavailable,
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol  = NULL,
#endif
  .send           = mcxa1xx_send,
  .txint          = mcxa1xx_txint,
  .txready        = mcxa1xx_txready,
  .txempty        = mcxa1xx_txempty,
};
#endif
...
{
  .setup          = mcxa1xx_dma_setup,
  .shutdown       = mcxa1xx_dma_shutdown,
  .attach         = mcxa1xx_attach,
  .detach         = mcxa1xx_detach,
  .ioctl          = mcxa1xx_ioctl,
  .receive        = mcxa1xx_dma_receive,
  .rxint          = mcxa1xx_dma_rxint,
  .rxavailable    = mcxa1xx_dma_rxavailable,
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol  = NULL,
#endif
  .send           = mcxa1xx_send,
  .txint          = mcxa1xx_dma_txint,
  .txready        = mcxa1xx_txready,
  .txempty        = mcxa1xx_txempty,
  .dmatxavail     = mcxa1xx_dma_txavailable,
  .dmasend        = mcxa1xx_dma_send,
};
#endif
...
#ifdef CONFIG_MCXA1XX_LPUART0
static struct mcxa1xx_uart_s g_lpuart0priv =
{
  .dev =
    {
      .recv     =
  ...
  .uartbase     = MCXA1XX_LPUART0_BASE,
  .uartnum      = 0,
  .baud         = CONFIG_LPUART0_BAUD,
  .irq          = MCXA156_IRQ_LPUART0,
...
#ifdef CONFIG_MCXA1XX_LPUART1
static struct mcxa1xx_uart_s g_lpuart1priv =
{
  .dev =
    {
      .recv     =
...
#  endif
  ...
  .uartbase     = MCXA1XX_LPUART1_BASE,
  .uartnum      = 1,
  .baud         = CONFIG_LPUART1_BAUD,
  .irq          = MCXA156_IRQ_LPUART1,
...
#ifdef CONFIG_MCXA1XX_LPUART2
static struct mcxa1xx_uart_s g_lpuart2priv =
{
  .dev =
    {
      .recv     =
...
#  endif
  ...
  .uartbase     = MCXA1XX_LPUART2_BASE,
  .uartnum      = 2,
  .baud         = CONFIG_LPUART2_BAUD,
  .irq          = MCXA156_IRQ_LPUART2,
...
#ifdef CONFIG_MCXA1XX_LPUART3
static struct mcxa1xx_uart_s g_lpuart3priv =
{
  .dev =
    {
      .recv     =
...
#  endif
  ...
  .uartbase     = MCXA1XX_LPUART3_BASE,
  .uartnum      = 3,
  .baud         = CONFIG_LPUART3_BAUD,
  .irq          = MCXA156_IRQ_LPUART3,
...
#ifdef CONFIG_MCXA1XX_LPUART4
static struct mcxa1xx_uart_s g_lpuart4priv =
{
  .dev =
    {
      .recv     =
...
#  endif
  ...
  .uartbase     = MCXA1XX_LPUART4_BASE,
  .uartnum      = 4,
  .baud         = CONFIG_LPUART4_BAUD,
  .irq          = MCXA156_IRQ_LPUART4,
...
#ifdef SERIAL_HAVE_RXDMA
static int mcxa1xx_dma_nextrx(struct mcxa1xx_uart_s *priv)
{
  int dmaresidual = mcxa1xx_dmach_getcount(priv->rxdma);
...
/****************************************************************************
 * Name: mcxa1xx_disableuartint
 ****************************************************************************/

static inline void mcxa1xx_disableuartint_nolock(struct mcxa1xx_uart_s *priv,
                                              uint32_t *ie)
{
  uint32_t regval;

  regval = mcxa1xx_serialin(priv, MCXA1XX_LPUART_CTRL_OFFSET);
  ...
  mcxa1xx_serialout(priv, MCXA1XX_LPUART_CTRL_OFFSET, regval);
}
                                       ...
                                       uint32_t *ie)
{
  ...
  mcxa1xx_disableuartint_nolock(priv, ie);
  spin_unlock_irqrestore(&priv->lock, flags);
}

/****************************************************************************
 * Name: mcxa1xx_restoreuartint
 ****************************************************************************/

static inline void mcxa1xx_restoreuartint_nolock(struct mcxa1xx_uart_s *priv,
                                              uint32_t ie)
{
  uint32_t regval;
                                       ...
                                       uint32_t ie)
{
  ...
  flags = spin_lock_irqsave(&priv->lock);
  mcxa1xx_restoreuartint_nolock(priv, ie);
  spin_unlock_irqrestore(&priv->lock, flags);
}

/****************************************************************************
 * Name: mcxa1xx_dma_setup
 *
 * Description:
...
static int mcxa1xx_dma_setup(struct uart_dev_s *dev)
{
  struct mcxa1xx_uart_s *priv = (struct mcxa1xx_uart_s *)dev;
#if defined(SERIAL_HAVE_RXDMA)
  struct mcxa1xx_edma_xfrconfig_s config;
#endif
  int result;
  ...
  if (priv->txch != 0)
    {
      if (priv->txdma == NULL)
        {
          priv->txdma = mcxa1xx_dmach_alloc(priv->txch, 0);
          if (priv->txdma == NULL)
            {
              return -EBUSY;
            }
      ...
      modifyreg32(priv->uartbase + MCXA1XX_LPUART_BAUD_OFFSET,
                  0, LPUART_BAUD_TDMAE);
    }
#endif
  ...
  if (priv->rxch != 0)
    {
      if (priv->rxdma == NULL)
        {
          priv->rxdma = mcxa1xx_dmach_alloc(priv->rxch, 0);
        ...
        {
          mcxa1xx_dmach_stop(priv->rxdma);
        }

      /* Configure for circular DMA reception into the RX FIFO */

      config.saddr  = priv->uartbase + MCXA1XX_LPUART_DATA_OFFSET;
      ...
      config.nbytes = 1;
#ifdef CONFIG_MCXA1XX_EDMA_ELINK
      config.linkch = 0;
#endif

      mcxa1xx_dmach_xfrsetup(priv->rxdma , &config);
      ...
      modifyreg32(priv->uartbase + MCXA1XX_LPUART_CTRL_OFFSET, 0,
                  LPUART_CTRL_PEIE       |
                  LPUART_CTRL_FEIE       |
                  LPUART_CTRL_NEIE       |
                  LPUART_CTRL_ILIE);
      ...
      mcxa1xx_dmach_start(priv->rxdma, mcxa1xx_dma_rxcallback, (void *)priv);
    }
#endif
...
static int mcxa1xx_setup(struct uart_dev_s *dev)
{
  ...
  ret = mcxa1xx_lpuart_configure(priv->uartbase, priv->uartnum, &config);

  priv->ie = mcxa1xx_serialin(priv, MCXA1XX_LPUART_CTRL_OFFSET) & \
             LPUART_ALL_INTS;
  return ret;
...
static void mcxa1xx_shutdown(struct uart_dev_s *dev)
{
  ...
  mcxa1xx_serialout(priv, MCXA1XX_LPUART_GLOBAL_OFFSET, LPUART_GLOBAL_RST);
}

/****************************************************************************
 * Name: mcxa1xx_dma_shutdown
 *
 * Description:
...
static void mcxa1xx_dma_shutdown(struct uart_dev_s *dev)
{
  ...
  if (priv->txch != 0)
    {
      mcxa1xx_dmach_stop(priv->txdma);

      /* Release the TX DMA channel */

      mcxa1xx_dmach_free(priv->txdma);
      priv->txdma = NULL;
    }
#endif
}
...
static int mcxa1xx_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(priv->irq, mcxa1xx_interrupt, dev);
  if (ret == OK)
    {
...
/****************************************************************************
 * Name: mcxa1xx_interrupt (and front-ends)
 *
 * Description:
...
static int mcxa1xx_interrupt(int irq, void *context, void *arg)
{
  ...
  struct mcxa1xx_uart_s *priv;
  uint32_t usr;
  uint32_t lsr;

  DEBUGASSERT(dev != NULL && dev != NULL);
  priv = (struct mcxa1xx_uart_s *)dev;

#if defined(CONFIG_PM) && CONFIG_MCXA1XX_PM_SERIAL_ACTIVITY > 0
  /* Repriv serial activity to the power management logic */

  pm_activity(PM_IDLE_DOMAIN, CONFIG_MCXA1XX_PM_SERIAL_ACTIVITY);
#endif
  ...
  usr  = mcxa1xx_serialin(priv, MCXA1XX_LPUART_STAT_OFFSET);
  ...
  if ((usr & LPUART_STAT_OR) != 0)
    {
      mcxa1xx_serialout(priv, MCXA1XX_LPUART_STAT_OFFSET,
                     LPUART_STAT_OR | lsr);
    }
  ...
  if ((usr & (LPUART_STAT_FE | LPUART_STAT_PF | LPUART_STAT_NF)) != 0)
    {
      ...
      mcxa1xx_serialin(priv, MCXA1XX_LPUART_DATA_OFFSET);
    }
  ...
  if ((usr & LPUART_STAT_IDLE) != 0)
    {
      ...
      mcxa1xx_dma_rxcallback(priv->rxdma, priv, false, LPUART_STAT_IDLE);
    }
#endif

  /* Handle incoming, receive bytes */

  if ((priv->ie & LPUART_CTRL_RIE) != 0 && mcxa1xx_rxavailable(&priv->dev))
    {
      uart_recvchars(dev);
    }
...
static int mcxa1xx_ioctl(struct file *filep, int cmd, unsigned long arg)
{
    ...
    defined(CONFIG_MCXA1XX_LPUART_SINGLEWIRE ) || \
    defined(CONFIG_MCXA1XX_LPUART_INVERT )
  ...
  switch (cmd)
    ...
    case TIOCSERGSTRUCT:
      {
         struct mcxa1xx_uart_s *user = (struct mcxa1xx_uart_s *)arg;
         if (!user)
           {
             ret = -EINVAL;
           }
           ...
           {
             memcpy(user, dev, sizeof(struct mcxa1xx_uart_s));
           }
       }
       break;
#endif
...
#ifdef CONFIG_MCXA1XX_LPUART_SINGLEWIRE
...
#ifdef CONFIG_MCXA1XX_LPUART_INVERT
    case TIOCSINVERT:
      {
        uint32_t ctrl;
        uint32_t stat;
        ...
        flags  = spin_lock_irqsave(&priv->lock);
        ctrl   = mcxa1xx_serialin(priv, MCXA1XX_LPUART_CTRL_OFFSET);
        stat   = mcxa1xx_serialin(priv, MCXA1XX_LPUART_STAT_OFFSET);
        regval = ctrl;
        ...
        */
        ...
        mcxa1xx_serialout(priv, MCXA1XX_LPUART_STAT_OFFSET, stat);
        mcxa1xx_serialout(priv, MCXA1XX_LPUART_CTRL_OFFSET, ctrl);
...
static int mcxa1xx_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  rxd     = mcxa1xx_serialin(priv, MCXA1XX_LPUART_DATA_OFFSET);
...
static bool mcxa1xx_rxavailable(struct uart_dev_s *dev)
{
  ...
  regval = mcxa1xx_serialin(priv, MCXA1XX_LPUART_WATER_OFFSET);
  return ((regval & LPUART_WATER_RXCOUNT_MASK) != 0);
}
#endif

/****************************************************************************
 * Name: mcxa1xx_dma_receive
 *
 * Description:
 *   Called (usually) from the interrupt level to receive one
...
#ifdef SERIAL_HAVE_RXDMA
static int mcxa1xx_dma_receive(struct uart_dev_s *dev, unsigned int *status)
{
  struct mcxa1xx_uart_s *priv = (struct mcxa1xx_uart_s *)dev;
  uint32_t nextrx             = mcxa1xx_dma_nextrx(priv);
  int c                       = 0;
   ...
   * driver must call mcxa1xx_dma_rxavailable prior to calling this
...
static bool mcxa1xx_dma_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (mcxa1xx_dma_nextrx(priv) != priv->rxdmanext);
}
#endif

/****************************************************************************
 * Name: mcxa1xx_dma_txcallback
 *
 * Description:
...
static void mcxa1xx_dma_txcallback(DMACH_HANDLE handle, void *arg, bool done,
                                  int result)
{
  struct mcxa1xx_uart_s *priv = (struct mcxa1xx_uart_s *)arg;
  ...
  mcxa1xx_dma_txavailable(&priv->dev);
}
#endif

/****************************************************************************
 * Name: mcxa1xx_dma_txavailable
 *
 * Description:
...
static void mcxa1xx_dma_txavailable(struct uart_dev_s *dev)
{
  ...
  if (mcxa1xx_dmach_idle(priv->txdma) == 0)
    {
...
static void mcxa1xx_dma_send(struct uart_dev_s *dev)
{
  ...
  config.daddr  = priv->uartbase + MCXA1XX_LPUART_DATA_OFFSET;
  config.soff   = 1;
  config.doff   = 0;
#ifdef CONFIG_MCXA1XX_EDMA_ELINK
  config.linkch  = 0;
  ...
  mcxa1xx_dmach_xfrsetup(priv->txdma, &config);
  ...
  mcxa1xx_dmach_start(priv->txdma, mcxa1xx_dma_txcallback, (void *)priv);
}
#endif
...
static void mcxa1xx_send(struct uart_dev_s *dev, int ch)
{
  ...
  mcxa1xx_serialout(priv, MCXA1XX_LPUART_DATA_OFFSET, (uint32_t)ch);
}

/****************************************************************************
 * Name: mcxa1xx_dma_txint
 *
 * Description:
...
static void mcxa1xx_dma_txint(struct uart_dev_s *dev, bool enable)
{
   ...
   * sequence. Furthermore we can use mcxa1xx_dma_txcallback() to handle
...
static bool mcxa1xx_txready(struct uart_dev_s *dev)
{
  ...
  regval = mcxa1xx_serialin(priv, MCXA1XX_LPUART_FIFO_OFFSET);
                                  ...
                                  int result)
{
  ...
  if (priv->rxenable && mcxa1xx_dma_rxavailable(&priv->dev))
    {
  ...
  sr = mcxa1xx_serialin(priv, MCXA1XX_LPUART_STAT_OFFSET);
...
void arm_earlyserialinit(void)
{
   ...
   * function mcxa1xx_lowsetup() earlier in the boot sequence.
   */
 ...
 *   that mcxa1xx_earlyserialinit was called previously.
 *
 ****************************************************************************/
...
void arm_serialinit(void)
{
  ...
  mcxa1xx_dma_setup(&CONSOLE_DEV.dev);
#endif
#endif
...
{
#ifdef CONSOLE_DEV
  struct mcxa1xx_uart_s *priv = (struct mcxa1xx_uart_s *)&CONSOLE_DEV;
  uint32_t ie;
  ...
  mcxa1xx_disableuartint(priv, &ie);
#endif

  arm_lowputc(ch);
#ifdef CONSOLE_DEV
  mcxa1xx_restoreuartint(priv, ie);
#endif
}
